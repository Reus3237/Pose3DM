...
from lib.model.Pose3DM import Pose3DM
...
def load_backbone(args):
    if not(hasattr(args, "backbone")):
        args.backbone = 'Pose3DM_B' # Default
    # if args.backbone == 'Pose3DM':
    #     from lib.model.Pose3DM import Pose3DM
    #     model_backbone = Pose3DM(
    #         num_frame=args.maxlen,
    #         num_joints=args.num_joints,
    #         in_chans=2,  # Number of input channels (e.g., 2 for 2D coordinates)
    #         embed_dim_ratio=128,
    #         mlp_ratio=args.mlp_ratio,
    #         depth=args.depth,
    #
    #     ).cuda()  # Make sure to move the model to GPU if you're using CUDA
    if args.backbone == 'Pose3DM_S':
        from lib.model.Pose3DM_S import Pose3DM
        model_backbone = Pose3DM(
            num_frame=args.maxlen,
            num_joints=args.num_joints,
            in_chans=2,  # Number of input channels (e.g., 2 for 2D coordinates)
    ...
    elif args.backbone == 'Pose3DM_B':
        from lib.model.Pose3DM_B import Pose3DM
        model_backbone = Pose3DM(
            num_frame=args.maxlen,
            num_joints=args.num_joints,
            in_chans=2,  # Number of input channels (e.g., 2 for 2D coordinates)
    ...
    elif args.backbone == 'Pose3DM_L':
        from lib.model.Pose3DM_L import Pose3DM
        model_backbone = Pose3DM(
            num_frame=args.maxlen,
            num_joints=args.num_joints,
            in_chans=2,  # Number of input channels (e.g., 2 for 2D coordinates)
