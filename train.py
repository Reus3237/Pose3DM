...
def parse_args():
    ...
    parser.add_argument("--config", type=str, default="configs/pose3d/Pose3DM_train_h36m_S.yaml",
                        help="Path to the config file.")
    parser.add_argument('-c', '--checkpoint', default='checkpoint/pose3d/MB_train_h36m', type=str, metavar='PATH',
...
def train_epoch(args, model_pos, train_loader, losses, optimizer, has_3d, has_gt):
    ...
    # 使用 tqdm 包装 train_loader 迭代器，显示进度条
    for idx, (batch_input, batch_gt) in tqdm(enumerate(train_loader), total=len(train_loader), desc="Training Epoch ",
                                             ncols=100):
...
def train_with_config(args, opts):
    opts.checkpoint = opts.checkpoint + '_' + datetime.datetime.fromtimestamp(get_beijing_timestamp()).strftime(
        '%Y_%m_%d_T_%H_%M_%S')
    # global log
    ...
    datareader = DataReaderH36M(n_frames=args.clip_len, sample_stride=args.sample_stride,
                                ...
                                dt_root=args.data_root, dt_file=args.dt_file)
    min_loss = 100000
    ...
    if not opts.evaluate:
        ...
        if args.train_2d:
            log.info(
                f'INFO: Training on {len(train_loader_3d)}(3D)+{len(instav_loader_2d) + len(posetrack_loader_2d)}(2D) batches')
        else:
        ...
        for epoch in range(st, args.epochs):
            ...
            if args.no_eval:
                ...
                log.info('[%d] time %.2f lr %f 3d_train %f e1 %f e2 %f' % (
                    ...
                    e1, e2))
                log.info(
                    f'Remaining training time: {datetime.timedelta(seconds=time.time() - start_time) * (args.epochs - epoch)}')
